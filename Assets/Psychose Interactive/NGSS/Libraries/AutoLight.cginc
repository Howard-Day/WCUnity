...
// ---- Screen space direction light shadows helpers (any version)
		...
		inline fixed unitySampleShadowNGSS(unityShadowCoord4 shadowCoord, float3 screenPos)
		{
		...
		#if defined(NGSS_SHADOWS_DITHERING_OPTIMIZED)//defined(SHADOWS_SOFT)
			float randPied = InterleavedGradientNoiseOptimized(screenPos.xy) * UNITY_TWO_PI;
		#else
			...
			return lerp(_LightShadowData.r, 1.0, PCF_FILTER_SPOT_OPTIMIZED(coord, diskRadius, randPied / max(1.0, screenPos.z), samplers));
		}
	...
	#endif // UNITY_SCREENSPACE_SHADOWS
...
uniform UNITY_DECLARE_SCREENSPACE_SHADOWMAP(NGSS_FrustumShadowsTexture);
//uniform UNITY_DECLARE_SCREENSPACE_TEXTURE(NGSS_FrustumShadowsTexture);
...
half UnityComputeForwardShadows(float2 lightmapUV, float3 worldPos, float4 screenPos)
{
	...
	realtimeShadowAttenuation = unitySampleShadowNGSS(mul(unity_WorldToShadow[0], unityShadowCoord4(worldPos, 1)), float3(screenPos.xy / screenPos.w, wDst));//NGSS OPTIMZIED DIRECTIONAL
#else
	...
	realtimeShadowAttenuation = unitySampleShadow(screenPos);//SCREEN SPACE SHADOWS	
#endif
	realtimeShadowAttenuation = lerp(realtimeShadowAttenuation, min(realtimeShadowAttenuation, saturate(UNITY_SAMPLE_SCREEN_SHADOW(NGSS_FrustumShadowsTexture, screenPos).r + NGSS_FRUSTUM_SHADOWS_OPACITY)), NGSS_FRUSTUM_SHADOWS_ENABLED);
#endif
	...
	//spot realtime shadow
		...
		float wDst = length(wDir);
		screenPos = ComputeScreenPos(UnityWorldToClipPos(worldPos));
		realtimeShadowAttenuation = UnitySampleShadowmapNGSS(spotShadowCoord, float3(screenPos.xy / screenPos.w, wDst));
	#else
		...
		float wDst = length(wDir);
		screenPos = ComputeScreenPos(UnityWorldToClipPos(worldPos));
		realtimeShadowAttenuation = UnitySampleShadowmapNGSS(worldPos - _LightPositionRange.xyz, float3(screenPos.xy / screenPos.w, wDst));
	#else
...
#	       define UNITY_TRANSFER_SHADOW(a, coord) {a._ShadowCoord.xy = coord * unity_LightmapST.xy + unity_LightmapST.zw; a._ShadowCoord.zw = ComputeScreenPos(a.pos).xy;}  
#		else
#	       define UNITY_TRANSFER_SHADOW(a, coord) a._ShadowCoord.xy = coord * unity_LightmapST.xy + unity_LightmapST.zw;
#		endif
...
#       if !defined(UNITY_HALF_PRECISION_FRAGMENT_SHADER_REGISTERS)
#			if defined(SHADOWS_SCREEN)
#	           define UNITY_TRANSFER_SHADOW(a, coord) a._ShadowCoord.zw = ComputeScreenPos(a.pos).xy;
#			else
#				define UNITY_TRANSFER_SHADOW(a, coord)
#			endif
#       else
#			if defined(SHADOWS_SCREEN)
#	           define UNITY_TRANSFER_SHADOW(a, coord) TRANSFER_SHADOW(a) a._ShadowCoord.zw = ComputeScreenPos(a.pos).xy;
#			else
#				define UNITY_TRANSFER_SHADOW(a, coord) TRANSFER_SHADOW(a)
#			endif
#       endif
...
#			if defined(SHADOWS_SCREEN)
#				define UNITY_SHADOW_ATTENUATION(a, worldPos) UnityComputeForwardShadows(0, worldPos, float4(a._ShadowCoord.zw, 0.0, UNITY_SHADOW_W(a.pos.w)))
#			else
