...
#endif
uniform float NGSS_LOCAL_NORMAL_BIAS = 0.01;
...
)
{
    ...
    #endif
	//v.vertex.xyz -= v.normal.xyz * NGSS_LOCAL_NORMAL_BIAS;
	...
	float shadow_bias = unity_LightShadowBias.z + NGSS_LOCAL_NORMAL_BIAS;//unity_LightShadowBias.z is not feed by renderer if light is set to spot or point :(
	...
	if (shadow_bias != 0.0)
    {
        ...
        float normalBias = shadow_bias * shadowSine;
		
		//return float2(shadowSine, min(2, shadowSine / shadowCos));//xy offsets
