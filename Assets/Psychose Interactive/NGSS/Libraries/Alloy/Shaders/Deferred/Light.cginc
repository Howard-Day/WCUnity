...
unity_v2f_deferred aVertexShader(
...
{
    ASurface s = aDeferredSurfaceShader(i);
	...
	d.shadow = NGSS_FRUSTUM_SHADOWS_ENABLED > 0.0 ? min(d.shadow, saturate(tex2D(NGSS_FrustumShadowsTexture, s.screenUv).r + NGSS_FRUSTUM_SHADOWS_OPACITY)) : d.shadow;//NEW addition
